...
func (g GameAoE4) CreateLogs(path string) (err error) {
	if err = os.WriteFile("session_data.txt", []byte("Session Data content"), os.ModePerm); err != nil {
		return
	}
	if err = os.WriteFile("warnings.log", []byte("Warnings content"), os.ModePerm); err != nil {
		return
	}
